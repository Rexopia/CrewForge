...
// in `~/.crewforge/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.crewforge/.secret_key`)
    key_path: PathBuf,
...
mod tests {
    ...
    fn decrypt_and_migrate_upgrades_legacy_xor() {
        ...
        assert!(new_value.starts_with("enc2:"));
        assert!(!SecretStore::needs_migration(&new_value));

        let (decrypted2, migrated2) = store.decrypt_and_migrate(&new_value).unwrap();
        assert_eq!(decrypted2, plaintext);
        assert!(migrated2.is_none());
    }
