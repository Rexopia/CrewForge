...
impl Provider for OpenRouterProvider {
    ...
    async fn chat_with_system(
        ...
        let credential = self
            .credential
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("OpenRouter API key not set. Set OPENROUTER_API_KEY."))?;
        ...
        let response = self
            ...
            .header("Authorization", format!("Bearer {credential}"))
            .header("HTTP-Referer", "https://github.com/crewforge/crewforge")
            .header("X-Title", "CrewForge")
            .json(&request)
            .send()
            .await?;
    ...
    async fn chat(
        ...
        let credential = self.credential.as_ref().ok_or_else(|| {
            anyhow::anyhow!("OpenRouter API key not set. Set OPENROUTER_API_KEY.")
        })?;
...
mod tests {
    use super::*;
    use crate::provider::traits::{ChatMessage, Provider};

    #[test]
    fn capabilities_report_native_tool_support() {
