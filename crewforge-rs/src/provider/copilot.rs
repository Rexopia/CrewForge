...
/// Tokens are cached to `~/.config/crewforge/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "crewforge")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("crewforge-copilot-{user}"))
            });
    ...
    const COPILOT_HEADERS: [(&'static str, &'static str); 4] = [
