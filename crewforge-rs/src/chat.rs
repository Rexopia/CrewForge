...
impl ChatRuntime {
    ...
    fn runtime_agent_name(&self, agent_id: &str) -> String {
        self.runtime_agents
            .iter()
    ...
    fn emit_agent_status(&self, agent_id: &str, status: &str, reason: Option<&str>) {
        self.send_display_line(DisplayLine::AgentStatus {
            agent: self.runtime_agent_name(agent_id),
            status: status.to_string(),
            reason: reason.map(|value| value.to_string()),
        });
    }

    fn agents_snapshot_rpc_event(&self) -> Value {
        let agents = self
            .runtime_agents
            .iter()
            .enumerate()
            .map(|(idx, agent)| {
                json!({
                    "name": agent.name,
                    "model": agent.model,
                    "contextDir": agent.runtime_dir.display().to_string(),
                    "agentIdx": idx,
                })
            })
            .collect::<Vec<_>>();
...
pub async fn run_chat(args: ChatArgs) -> Result<()> {
    ...
    let initial_snapshot_for_stdout = if (!is_tty || args.rpc_jsonl) && resumed_from.is_some() {
    ...
    let mut maybe_msg_rx = Some(msg_rx);
    if !is_tty && !args.rpc_jsonl {
        let _ = maybe_msg_rx.take();
    }
    ...
    let show_startup_banner = args.rpc_jsonl || !is_tty || resumed_from.is_none();
    if show_startup_banner {
        runtime.print_system_line(&format!("Root: {}", runtime.room.workspace_dir.display()));
        let agent_lines = runtime
            .runtime_agents
            .iter()
            .map(|item| format!("{}[{}]", item.name, item.model))
            .collect::<Vec<_>>();
        if agent_lines.is_empty() {
            runtime.print_system_line("Agents:");
        } else {
            runtime.print_system_line(&format!("Agents:\n  {}", agent_lines.join("\n  ")));
        }
    }
    ...
    if is_tty && !args.rpc_jsonl {
        ...
        .await?;
    } else if args.rpc_jsonl {
        let msg_rx = maybe_msg_rx
            .take()
            .ok_or_else(|| anyhow!("internal error: missing RPC display receiver"))?;
        run_jsonl_rpc_loop(
            runtime.clone(),
            msg_rx,
            stop_flag.clone(),
        ...
        run_plain_stdin_loop(runtime.clone(), stop_flag.clone()).await?;
    }
    ...
    let resume_hint = build_resume_hint_command(&args, &app_session_id);
    if args.rpc_jsonl {
        emit_rpc_event(json!({
            "type": "session.ended",
            "reason": "normal_exit",
        ...
        runtime.print_plain_line(&format!("Resume this session with: {resume_hint}"));
    }
    Ok(())
}

async fn run_plain_stdin_loop(runtime: Arc<ChatRuntime>, stop_flag: Arc<AtomicBool>) -> Result<()> {
...
async fn run_jsonl_rpc_loop(
    ...
    emit_rpc_event(json!({
        ...
        "sessionFile": runtime.session_file_path().display().to_string(),
        "human": runtime.human_name(),
        "resumedFrom": resumed_from.map(|path| path.display().to_string()),
    }))?;
    emit_rpc_event(runtime.agents_snapshot_rpc_event())?;
    ...
    loop {
        line.clear();
        tokio::select! {
            maybe_display = msg_rx.recv() => {
                match maybe_display {
                    Some(display) => {
                        emit_rpc_event(display_line_to_rpc_event(display))?;
                    }
                    None => break,
                }
            }
            read = reader.read_line(&mut line) => {
                ...
                let command = match parse_rpc_command(raw) {
                    Ok(command) => command,
                    Err(error) => {
                        emit_rpc_event(json!({
                            "type": "error",
                            "message": to_single_line_error(&error.to_string()),
                        }))?;
                        continue;
                    }
                };
                ...
                match command {
                    ...
                    RpcCommand::SlashCommand(command) => {
                        let command = command.trim();
                        if command.is_empty() {
                            emit_rpc_event(json!({
                                "type": "error",
                                "message": "rpc slash_command requires non-empty command",
                            }))?;
                            continue;
                        }
                        let user_input = if command.starts_with('/') {
                            command.to_string()
                        } else {
                            format!("/{command}")
                        };
...
fn parse_rpc_command(raw: &str) -> Result<RpcCommand> {
    let value: Value = serde_json::from_str(raw).context("invalid rpc json command")?;
    let command_type = value
        .get("type")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("rpc command missing string field: type"))?;
    ...
    match command_type {
        "input" => {
            let text = value
                .get("text")
                .and_then(Value::as_str)
                .ok_or_else(|| anyhow!("rpc input command requires string field: text"))?;
            Ok(RpcCommand::Input(text.to_string()))
        }
        "slash_command" => {
            let command = value
                .get("command")
...
fn display_line_to_rpc_event(line: DisplayLine) -> Value {
    match line {
        DisplayLine::System(text) => {
            if let Some(message) = text.strip_prefix("[warning] ") {
                json!({
                    "type": "warning",
                    "message": message,
                })
            } else if let Some(message) = text.strip_prefix("[error] ") {
                json!({
                    "type": "error",
                    "message": message,
                })
        ...
        DisplayLine::Human { ts, speaker, text } => json!({
            "type": "line",
            "role": "human",
            "ts": ts,
            "speaker": speaker,
...
fn emit_rpc_event(event: Value) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn parse_rpc_input_command() {
        let command =
            parse_rpc_command(r#"{"type":"input","text":"hello"}"#).expect("parse input command");
        assert_eq!(command, RpcCommand::Input("hello".to_string()));
    }

    #[test]
    fn parse_rpc_slash_command() {
        let command = parse_rpc_command(r#"{"type":"slash_command","command":"help"}"#)
            .expect("parse slash command");
        assert_eq!(command, RpcCommand::SlashCommand("help".to_string()));
    }
    ...
    #[test]
    fn parse_rpc_exit_command() {
        let command = parse_rpc_command(r#"{"type":"exit"}"#).expect("parse exit command");
        assert_eq!(command, RpcCommand::Exit);
    }

    #[test]
    fn parse_rpc_command_rejects_missing_type() {
        let error = parse_rpc_command(r#"{"text":"hello"}"#).expect_err("missing type should fail");
        assert!(error.to_string().contains("missing string field: type"));
    }

    #[test]
    fn parse_rpc_command_rejects_unknown_type() {
        let error =
            parse_rpc_command(r#"{"type":"unknown"}"#).expect_err("unknown type should fail");
        assert!(error.to_string().contains("unsupported rpc command type"));
    }

    #[test]
    fn display_line_warning_maps_to_warning_event() {
        let event = display_line_to_rpc_event(DisplayLine::System(
            "[warning] profile Codex is unavailable".to_string(),
        ));
        assert_eq!(event["type"], "warning");
        assert_eq!(event["message"], "profile Codex is unavailable");
    }

    #[test]
    fn display_line_agent_status_maps_to_status_event() {
        let event = display_line_to_rpc_event(DisplayLine::AgentStatus {
