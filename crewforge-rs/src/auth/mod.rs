...
pub fn normalize_provider(provider: &str) -> Result<String> {
    ...
    match normalized.as_str() {
        ...
        "anthropic" | "claude" | "claude-code" => Ok("anthropic".to_string()),
        "gemini" | "google" | "vertex" => Ok("gemini".to_string()),
        other if !other.is_empty() => Ok(other.to_string()),
        _ => anyhow::bail!("Provider name cannot be empty"),
    }
}

/// Return the default state directory for crewforge (~/.crewforge).
pub fn default_state_dir() -> PathBuf {
    directories::UserDirs::new()
        .map(|dirs| dirs.home_dir().join(".crewforge"))
        .unwrap_or_else(|| PathBuf::from(".crewforge"))
}
